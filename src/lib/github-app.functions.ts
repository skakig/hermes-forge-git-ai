...
// Lightweight config + reachability probe for the Hermes backend.
// Used by the Settings page so users can see at a glance whether the
// agent runtime is configured and responsive.
...
export const checkHermesHealth = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .handler(async (): Promise<HermesHealth> => {
    const checkedAt = new Date().toISOString();
    const url = process.env.HERMES_API_URL ?? null;
    const key = process.env.HERMES_API_KEY ?? null;
    const hasUrl = !!url;
    const hasKey = !!key;
    if (!hasUrl || !hasKey) {
      return {
    ...
    try {
      ...
      return {
        ...
        message: res.ok
          ...
          : `Hermes returned ${res.status}: ${text.slice(0, 160) || res.statusText}`,
      };
    } catch (e) {
      return {
        ok: false,
        ...
        url: base,
        status: null,
        message: `Network error reaching Hermes: ${e instanceof Error ? e.message : String(e)}`,
      };
    }
  });